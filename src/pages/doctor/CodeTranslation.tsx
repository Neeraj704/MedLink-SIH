            ...
            <CardContent className="p-4">
              ...
              {results.length > 0 && (
                <div className="mt-3 border rounded-lg divide-y max-h-[500px] overflow-y-auto bg-popover shadow-xl border-border">
                  {results.map((row, i) => (
                    <button key={i} onClick={() => addDiagnosis(row)} className="w-full text-left p-4 hover:bg-accent/50 transition-colors flex flex-col gap-1 border-b last:border-b-0">
                      ...
                      <div className="grid md:grid-cols-3 gap-4">
                        {[
                          ...
                          <Tooltip key={j}>
                            ...
                            <TooltipContent><p className="text-xs">{system.label} equivalent for {row.ICD11_Title}</p></TooltipContent>
                          </Tooltip>
                        ))}
                      </div>
                    </CardContent>
