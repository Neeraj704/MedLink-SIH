...
import { 
  Activity, Mail, Shield, CreditCard, Fingerprint, 
  ArrowRight, Eye, EyeOff, Clock, ArrowLeft 
} from 'lucide-react';
import { Button } from '@/components/ui/button';
import { Input } from '@/components/ui/input';
import { useAuth, type UserRole, type AuthMethod } from '@/contexts/AuthContext';
...
export default function AuthPage() {
  ...
  const doctorMethods: { method: AuthMethod; icon: typeof Mail; label: string; placeholder: string }[] = [
    { method: 'gmail', icon: Mail, label: 'Continue with Gmail', placeholder: 'Enter your Gmail address' },
    { method: 'hpr', icon: Shield, label: 'Continue with HPR ID', placeholder: 'Enter your HPR ID' },
  ];

  const patientMethods: { method: AuthMethod; icon: typeof CreditCard; label: string; placeholder: string }[] = [
    { method: 'abha', icon: CreditCard, label: 'Continue with ABHA ID', placeholder: 'Enter your 14-digit ABHA ID' },
    { method: 'aadhaar', icon: Fingerprint, label: 'Continue with Aadhaar', placeholder: 'Enter your 12-digit Aadhaar' },
  ];
    ...
    <div className="min-h-screen flex bg-background">
      ...
      <div className="hidden lg:flex lg:w-1/2 bg-primary relative items-center justify-center p-12">
        <div className="text-primary-foreground max-w-md">
          ...
          <h2 className="text-3xl font-bold mb-4">
            {step === 'role' ? 'Welcome to the future of healthcare.' :
             role === 'doctor' ? 'Empowering healthcare professionals.' :
             'Your health records, simplified.'}
          </h2>
          <p className="text-primary-foreground/70 text-lg">
            {step === 'role' ? 'FHIR-compliant records, Ayush integration, and seamless patient care.' :
             role === 'doctor' ? 'Manage patients, encode FHIR records, and translate disease codes across Ayush systems.' :
             'Access prescriptions in your language, find doctors nearby, and download records instantly.'}
          </p>
          <div className="mt-12 space-y-3">
            {['HL7 FHIR R4 Compliant', 'ICD-11 & Ayush Mapping', 'End-to-end Encrypted'].map((item, i) => (
      ...
      <div className="flex-1 flex items-center justify-center p-6 md:p-12">
        <div className="w-full max-w-md">
          {step !== 'role' && (
            <button onClick={() => { setStep(step === 'otp' ? 'login' : 'role'); setOtp(['','','','','','']); setMethod(null); }}
          ...
          <AnimatePresence mode="wait">
            {step === 'role' && (
              <motion.div key="role" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }}>
                ...
                <div className="grid grid-cols-2 gap-4">
                  {[
                    { r: 'doctor' as UserRole, label: 'Doctor', desc: 'Manage patients & records', icon: '🩺' },
                    { r: 'patient' as UserRole, label: 'Patient', desc: 'View records & prescriptions', icon: '🏥' },
            ...
            {step === 'login' && (
              <motion.div key="login" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }}>
                ...
                <p className="text-muted-foreground mb-8">Choose your authentication method</p>
                  ...
                  <div className="space-y-4">
                    <div className="relative">
                      <Input
                        ...
                        placeholder={methods.find(m => m.method === method)?.placeholder}
                        value={identifier}
                        onChange={(e) => setIdentifier(e.target.value)}
                        className="pr-10 h-12"
                      />
                      <button onClick={() => setShowId(!showId)} className="absolute right-3 top-1/2 -translate-y-1/2 text-muted-foreground hover:text-foreground">
                    ...
                    <button className="text-xs text-primary hover:underline">Forgot your ID?</button>
                  </div>
                )}
              </motion.div>
            )}
            ...
            {step === 'otp' && (
              <motion.div key="otp" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }}>
                ...
                <p className="text-muted-foreground mb-8">
                  Enter the 6-digit code sent to your registered {method === 'gmail' ? 'email' : 'mobile'}
                </p>
                ...
                <p className="text-xs text-muted-foreground text-center mt-6">
                  Hint: Use <span className="font-mono font-semibold text-foreground">123123</span> to verify
                </p>
              </motion.div>
            )}
          </AnimatePresence>
